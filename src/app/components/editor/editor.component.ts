...
@Component({
  ...
  template: `
    <div class="flex flex-col h-full bg-[var(--bg-primary)]">
      ...
      >
        ...
        <div
          ...
          [class.bg-[var(--bg-primary)]]="
            projectState.activeFilePath() === file.path
          "
      ...
      <div
        class="flex items-center gap-1 px-2 py-1.5 bg-[var(--bg-tertiary)] border-b border-[var(--border-color)] flex-wrap"
      >
        <!-- Text Style Group -->
        <div
          class="flex items-center gap-0.5 pr-2 border-r border-[var(--border-color)]"
        >
          <button
            class="toolbar-btn"
        ...
        <div class="flex items-center gap-0.5 px-2 border-r border-[#3c3c3c]">
          ...
          <button
            class="toolbar-btn"
            [class.active]="isAligned('justify')"
...
export class EditorComponent implements OnDestroy {
  ...
  private calculateLineCount(editor: Editor): void {
    const editorElement =
      this.editorContainer?.nativeElement.querySelector(".tiptap");
    if (!editorElement) {
    ...
    if (!this.cachedLineHeight) {
      ...
      this.cachedLineHeight =
        parseFloat(computedStyle.lineHeight) || fontSize * 1.9;
    }
