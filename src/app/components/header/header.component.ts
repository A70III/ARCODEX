  ...
  template: `
    ...
    >
      ...
      <div class="flex items-center h-full pl-2">
        ...
        <div class="flex items-center h-full -ml-1">
          <div class="relative group h-full flex items-center">
            ...
            @if (activeMenu() === 'file') {
            <div
              class="absolute top-full left-0 mt-1 w-56 bg-[var(--bg-secondary)] border border-[var(--border-light)] shadow-[0_4px_10px_rgba(0,0,0,0.5)] rounded-sm py-1 z-50"
            >
          ...
          <div class="relative group h-full flex items-center">
            ...
            @if (activeMenu() === 'edit') {
            <div
              class="absolute top-full left-0 mt-1 w-48 bg-[var(--bg-secondary)] border border-[var(--border-light)] shadow-lg rounded-sm py-1 z-50"
            >
              <button class="menu-item" (click)="triggerEdit('Undo')">
                <span>Undo</span><span class="shortcut">Ctrl+Z</span>
              </button>
              <button class="menu-item" (click)="triggerEdit('Redo')">
                <span>Redo</span><span class="shortcut">Ctrl+Y</span>
              </button>
              <div class="menu-divider"></div>
              <button class="menu-item" (click)="triggerEdit('Cut')">
                <span>Cut</span><span class="shortcut">Ctrl+X</span>
              </button>
              <button class="menu-item" (click)="triggerEdit('Copy')">
                <span>Copy</span><span class="shortcut">Ctrl+C</span>
              </button>
              <button class="menu-item" (click)="triggerEdit('Paste')">
          ...
          <div class="relative group h-full flex items-center">
            ...
            <div
              class="absolute top-full left-0 mt-1 w-48 bg-[var(--bg-secondary)] border border-[var(--border-light)] shadow-lg rounded-sm py-1 z-50"
            >
              <button class="menu-item" (click)="toggleSidebar()">
                <span>Toggle Sidebar</span><span class="shortcut">Ctrl+B</span>
              </button>
              <button class="menu-item" (click)="toggleInfoPanel()">
                <span>Toggle Info Panel</span>
              </button>
              <div class="menu-divider"></div>
              <button class="menu-item" (click)="toggleFocusMode()">
                <span>{{
                  projectState.focusMode() ? "Exit Focus Mode" : "Focus Mode"
                }}</span>
      ...
      <div class="flex items-center h-full">
        ...
        <div
          class="flex items-center mr-2 px-2 border-r border-[var(--border-color)] h-[16px]"
        >
          <!-- Focus Mode Button -->
          <button
          ...
          <button
            ...
            (click)="toggleSidebar()"
          >
            <span class="material-icons text-[18px] transform rotate-180"
              >view_sidebar</span
            >
          </button>
          <button
        ...
        <div class="flex items-center h-full">
          <button
            class="window-control hover:bg-[var(--bg-hover)]"
          ...
          <button
            class="window-control hover:bg-[var(--error)] hover:text-white group"
            (click)="closeWindow()"
            title="Close"
          >
            <svg
